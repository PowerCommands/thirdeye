...
namespace PainKiller.ThirdEyeAgentCommands.Commands
{
    ...
    public class ComponentCommand(string identifier, PowerCommandsConfiguration configuration) : ThirdEyeBaseCommando(identifier, configuration)
    {
        public override RunResult Run()
        {
            ...
            {
                Console.Clear();
                Console.WriteLine("➡ \"Type to filter results, press ENTER to select, BACKSPACE to delete:");
                ...
                if (filteredComponents.Count == 0) Console.WriteLine("No matching result...");
                else
                {
                ...
                if (key.Key == ConsoleKey.Backspace && inputBuffer.Length > 0) inputBuffer = inputBuffer.Substring(0, inputBuffer.Length - 1);
                else if (!char.IsControl(key.KeyChar)) inputBuffer += key.KeyChar;
            }
            if (filteredComponents.Count > 0)
            {
                ...
                var searchQuestion = DialogService.YesNoDialog($"Do you want to do a search for all versions {component.Name}?");
        ...
        private void ProjectSearch(ThirdPartyComponent component, bool detailedSearch)
        {
            ...
            PresentationManager.DisplayProjects(projects, repos, teams, devProjects);
        }
    }
}