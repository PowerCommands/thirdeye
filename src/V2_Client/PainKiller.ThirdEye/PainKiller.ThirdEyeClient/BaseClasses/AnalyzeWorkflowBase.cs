...
public class AnalyzeWorkflowBase(IConsoleWriter writer, CommandPromptConfiguration configuration)
{
    ...
    {
        var teams = FilterService.Service.GetTeams(configuration.ThirdEye.Teams.ToList()).ToList();
    ...
    {
        var workspaces = FilterService.Service.GetWorkspaces(configuration.ThirdEye.Workspaces.ToList(), selectedTeam).ToList();
    ...
    public virtual ComponentCve? ViewCveDetails(List<ComponentCve> componentCves)
    {
        ...
        var cveFetcher = new CveFetcherManager(CveStorageService.Service, configuration.ThirdEye.Nvd, apiKey, writer);
    ...
    {
        writer.WriteDescription(cveEntry.Id, $"{cve.Name} {cve.Version} CVEs: {cve.CveEntries.Count}" );
